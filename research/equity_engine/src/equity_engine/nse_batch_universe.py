    ...
    def _get(self, url: str) -> bytes:
        ...
        for attempt in range(1, self.max_attempts + 1):
            try:
                ...
                last_error = RuntimeError(f"transient NSE HTTP {response.status_code} for {url}")
    ...
    def build(
        ...
        *,
        ...
        for report_date in dates:
            ...
            except Exception as exc:  # noqa: BLE001 - each date is recorded fail-closed
                failures.append(
                    NseBatchFailure(
                        report_date=report_date,
        ...
        return NseBatchUniverseResult(
            ...
            holiday_dates=tuple(day for day in calendar.holiday_dates if start <= day <= end),
            excluded_special_session_dates=tuple(
                day for day in calendar.excluded_special_session_dates if start <= day <= end
            ),
